...
#     """
#     GET /models
#     → returns JSON { "models": [<model-name>, …] }
#     """
...
#             for c in chats
#         ])
#     else:  # POST → create new chat
#         chat = Chat()
#         db.session.add(chat)
#         db.session.commit()
#         return jsonify({'id': chat.id, 'created_at': chat.created_at.isoformat()}), 201
#
...
#
# # —————— Message endpoints ——————
# @app.route('/chats/<int:chat_id>/messages', methods=['GET', 'POST', 'DELETE'])
...
#         db.session.commit()
#         return '', 204
#     else:  # POST → add a new message
#         data = request.get_json()
#         sender = data.get('sender')
#         text = data.get('text', '')
#         if sender not in ('user', 'bot'):
#             return jsonify({'error': 'Invalid sender'}), 400
#         msg = Message(chat_id=chat.id, sender=sender, text=text)
#         db.session.add(msg)
#         db.session.commit()
#         return jsonify({
