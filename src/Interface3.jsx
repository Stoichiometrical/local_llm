        ...
        <div className={`relative px-4 py-3 rounded-xl shadow ${
          ...
          <div className={`absolute bottom-0 ${isAI ? 'right-0' : 'left-0'} opacity-0 group-hover:opacity-100 transition-opacity translate-y-full mb-2 flex space-x-1`}>
            <div className="bg-white dark:bg-gray-700 shadow-lg rounded-full px-2 py-1 flex space-x-1">
              <button
                onClick={handleCopy}
                className="p-1 hover:bg-gray-100 dark:hover:bg-gray-600 rounded-full flex items-center gap-1"
              >
                {copied ? <Check size={14} /> : <Copy size={14} />}
              </button>
              {isAI && (
        ...
        <ul className="flex-1 overflow-y-auto pt-2 px-2">
          ...
          {filteredChats.map(chat=> (
            <li
              ...
              className={`flex justify-between items-center px-3 py-2.5 rounded-lg cursor-pointer mb-1 transition-colors ${chat.id===selectedChat?'bg-violet-100 dark:bg-violet-900/40 text-violet-900 dark:text-violet-100':'hover:bg-gray-100 dark:hover:bg-gray-700'}`}
            >
              ...
              <button onClick={e=>deleteChat(chat.id,e)} className="p-1 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600">
                <Trash2 size={14}/>
              </button>
            </li>
          ))}
      ...
      <div className="flex flex-col flex-1 max-h-screen overflow-hidden">
        <header className="flex items-center justify-between h-16 px-4 sm:px-6 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm z-10">
          <div className="flex items-center">
            {isMobile ? (
              <button onClick={()=>setSidebarOpen(true)} className="p-2 mr-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700">
                <Menu size={20}/>
              </button>
            ) : (
              <button onClick={()=>setSidebarOpen(!sidebarOpen)} className="p-2 mr-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700">
        ...
        {showSettings&&(
          <div className="p-4 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={()=>setShowSettings(false)} className="p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700">
                <X size={18}/>
              </button>
            </div>
            ...
            <div className="max-w-3xl mx-auto">
              ...
              {!autoScrollEnabled && messages.length > 0 && (
                <button
                  ...
                  className="fixed bottom-24 right-6 bg-violet-600 text-white p-2 rounded-full shadow-lg hover:bg-violet-700"
                >
                  <ChevronDown size={20}/>
                </button>
              )}
          ...
          <div className="max-w-3xl mx-auto">
            <div className="flex items-end space-x-2">
              ...
              <button
                ...
                className={`p-2 rounded-full ${loading||!input.trim()?'bg-gray-300 dark:bg-gray-600 text-gray-500 dark:text-gray-400':'bg-violet-600 hover:bg-violet-700 text-white'} transition-colors`}
              >
